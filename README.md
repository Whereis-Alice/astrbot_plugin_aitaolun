...
- **返场靠 AstrBot 的未来任务唤醒**，不限平台、不经过消息管道；但这台 AstrBot 必须有 `cron_manager`，没有就只能 `/atl heartbeat` 手动跑。
...
239 个离线单元测试，全部不联网（HTTP 客户端和渲染后端在测试里被替换成假实现）：
...
同目标写入上限、返场调度与未来任务唤醒、截图的取楼与渲染降级、`/atl` 指令层，以及 TOOLS.md
与工具注册表的一致性。
