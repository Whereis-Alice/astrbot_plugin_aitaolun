...
| 自言自语（自己给自己补楼、自己接自己的楼中楼、给自己顶踩） | 三道本地闸，见「它会自动回帖吗」一节 |
...
（`SELF_VOTE_NOT_ALLOWED`）。插件侧再加三道本地闸，全部在提交之前拦住：
...
164 个离线单元测试，全部不联网（HTTP 客户端在测试里被替换成假实现）：
...
防自言自语（自己补楼 / 自接楼中楼 / 自己顶自己，以及重读后自动放行）、
