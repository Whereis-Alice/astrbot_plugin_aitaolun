...
截图这一版修了三处「看着不对」的地方，另外把不通的那条返场唤醒路彻底删掉。

- **头像终于画出来了**：之前只认一个字段名，站点各接口叫法不一（`avatar_url` / `author_avatar` / 嵌套在 `author` 里），一律认不出来就画首字母方块。现在按「已知键 → 嵌套 dict → 键名含 avatar 的兜底」三级找；payload 里压根没带头像的作者，再用公开接口 `GET /agents/{name}` 补一次（去重、6 小时缓存、单张图最多补 10 个、失败就退回方块，绝不拖垮截图）
- 顺手把头像 URL 改写成站点的小方图变体（`/img/<id>.webp` → `/avatar/v1/<id>.webp`），单张头像 75KB → 15KB
- 信息流、搜索结果、通知这三种视图以前完全不画头像，现在都画了
- **楼号对上了**：站点把主题本体算成 1 楼、回复从 2 开始，插件之前按列表下标从 1 编号，整体差一层。现在按 `floor_count` 与列表长度反推真实起点，楼层自带的显式楼号优先，但能识别出「这个楼号明显是列表下标」的情况并纠正；任何情况下都不会再印出「? 楼」
- **主题截图补上了根帖正文**：接口的楼层列表不含主题本体，以前截图直接从 2 楼开始，看着像内容丢了。现在自己画一张「1 楼 · 楼主」卡片，正文为空或接口本来就把根帖塞进列表时不重复画
- 截图标题去掉 `【爱讨论·截图】` 前缀，只留主题名、吧名、楼层数和链接
- **删掉伪造消息进管道那条返场唤醒路**（`StarTools.create_event(is_wake=True)`）。它过不了 `WakingCheckStage`，表现成「插件说发了、bot 毫无反应」，留着只会误导。返场现在只有 AstrBot 未来任务这一条路：没有 `cron_manager` 就在 `/atl status`、`/atl bind`、`/atl diag` 里直接标红说清楚，让人升级框架或先用 `/atl heartbeat` 手动跑
- 随之移除配置 `heartbeat_wake_mode`、`heartbeat_wake_prefix`（留着也没有第二条路可选），`docs/heartbeat.md` 重写
- 测试 232 → 239
